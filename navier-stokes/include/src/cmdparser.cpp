...
int CmdOptionBasic::parse (int cur_arg_index, int argc, const char** argv, bool errorOnDup)
{
    ...
    {
        bool bSkipAssign = false;

        if(m_parsed)
        {
            if (errorOnDup)     // option duplication is not allowed in this case
                throw CmdParser::Error(
                    "Option duplication: " + cur_arg_value
                    );
            else               // just skip duplicating option in this case
                bSkipAssign = true;
        }
...
CmdParserWithHelp::CmdParserWithHelp (int argc, const char** argv) :
    CmdParser(argc, argv),
    help(
        *this,
        'h',
...
CmdParserCLDeviceType::CmdParserCLDeviceType (int argc, const char** argv) :
    CmdParser(argc, argv),
...
CmdParserCLCommon::CmdParserCLCommon(int argc, const char** argv) :
    CmdParser(argc, argv),
    CmdParserWithHelp(argc, argv),
    CmdParserCLDeviceType(argc, argv),
    device(
        *this,
        'd',
        "device",
